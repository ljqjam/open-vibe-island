            ...
            if model.hiNotificationEnabled {
                ...
                Section {
                    ...
                    if model.hiInteractiveApprovalEnabled {
                        ...
                        LabeledContent("User Access Token") {
                            SecureField("卡片按钮所需，留空则用文本", text: Binding(
                                get: { model.hiUserAccessToken },
                                set: { model.hiUserAccessToken = $0 }
                            ))
                            .textFieldStyle(.plain)
                            .multilineTextAlignment(.trailing)
                        }
                    ...
                    if model.hiInteractiveApprovalEnabled {
                        Text("需在控制台「应用号 AI → 事件订阅」选 WebSocket、订阅 `asn:bot.chat.message:v1` 并发布。之后在 Hi 里回复 y 批准 / n 拒绝（多条待审批时回复: y <短码>）。填了「卡片模板 ID」+「User Access Token」会推送带按钮的卡片（点按钮 = 发送 y/n），否则用纯文本。多人共用同一机器人时，只有「审批推送的接收人」本人回复才会被采纳，不会互相串。")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
